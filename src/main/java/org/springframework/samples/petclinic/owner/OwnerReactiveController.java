...
public class OwnerReactiveController {
    ...
    private final OwnerReactiveRepository ownerRepo;
    
    /**
     * Injection with controller
     */
    public OwnerReactiveController(OwnerReactiveRepository repo) {
        this.ownerRepo = repo;
    }
    ...
    @ApiOperation(value= "Read all owners in database", response=Vet.class)
    @ApiResponses({
    ...
    public Mono<ResponseEntity<Owner>> getVet(@PathVariable("ownerId") @Parameter(
        ...
        return ownerRepo.findById(UUID.fromString(ownerId))
    ...
    public Mono<ResponseEntity<Owner>> createVet(UriComponentsBuilder ucBuilder, @RequestBody OwnerCreationDto dto) {
      ...
      return ownerRepo.save(input).map(created -> ResponseEntity
                              .created(ucBuilder.path("/api/owners/{id}").buildAndExpand(created.getId()).toUri())
                              .body(created));
    }
    ...
    public Mono<ResponseEntity<Owner>> upsert(
      ...
      return ownerRepo.save(owner).map(created -> ResponseEntity
              .created(ucBuilder.path("/api/owners/{id}").buildAndExpand(created.getId()).toUri())
              .body(created));
    }
    ...
    public Mono<ResponseEntity<Void>> deleteById(@PathVariable("ownerId") @Parameter(
            required = true,example = "1ff2fbd9-bbb0-4cc1-ba37-61966aa7c5e6",
            description = "Unique identifier of a owner") String ownerId) {
        return ownerRepo.delete(new Owner(ownerId)).map(v -> new ResponseEntity<Void>(HttpStatus.NO_CONTENT));
    }
}
